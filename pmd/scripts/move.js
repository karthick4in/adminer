...
function positionSave(){
    /// 
    let postion  = Get_url_pos();
    localStorage.setItem("postion",JSON.stringify( postion) );
}

function  loadPostion (){

    let returnDataStr =  localStorage.getItem("postion") || "{}";
    if( returnDataStr != "{}"){
        let returnDataJson = JSON.parse(returnDataStr) ;
        table_postion = returnDataJson
    }
}
...
{
    Circle(x1, y1, 3, 3, color_line);//"rgba(0,0,255,1)"
    Rect(x2 - 1, y2 - 2, 4, 4, color_line); //"rgba(0,0,255,1)"
...
function Get_url_pos()
{
    ...
    for (key in j_tabs) {
        let tData ={}
        tData["x"] =  parseInt(document.getElementById(key).style.left);
        tData["y"] =  parseInt(document.getElementById(key).style.top);
        tData["v"] =  (document.getElementById('id_tbody_' + key).style.display == 'none' ? 0 : 1);
        tData["h"] =  (document.getElementById('check_vis_' + key).checked ? 1 : 0);
        jsonData[key] = tData;
            ...
            for (key2 in contr[K][key])
                for (key3 in contr[K][key][key2]) {
                    ...
                    if (!selected && Glob_X > x1 - 10 && Glob_X < x1 + 10 && Glob_Y > y1 - 7 && Glob_Y < y1 + 7) {
                        Line0(x1 - sm_x, y1 - sm_y, x2 - sm_x, y2 - sm_y, generateRGBAColor(key3)); // "rgba(255,0,0,1)"
                        ...
                        let colorA  = generateRGBAColor(convertStringToNumberInRange(key2));
                        Line0(x1 - sm_x, y1 - sm_y, x2 - sm_x, y2 - sm_y, colorA); // "rgba(255,0,0,1)"
                    }
                }
    if (selected) {
        // select relations
...
// -----------------------------------------------------
function generateRGBAColor(value) {
  const colors = [
    [255, 0, 0],     // Red
    [0, 255, 0],     // Green
    [0, 0, 255],     // Blue
    ...
    [128, 128, 128], // Gray
    [192, 192, 192], // Silver
    [128, 255, 0],   // Lime (Chartreuse)
  ];

  const index = (value - 1) % colors.length;
  const [red, green, blue] = colors[index];

  const rgbaColor = `rgba(${red}, ${green}, ${blue}, 1)`;
  return rgbaColor;
}
function convertStringToNumberInRange(str) {
  let sum = 0;
  for (let i = 0; i < str.length; i++) {
    sum += str.charCodeAt(i);
  }
  const valueInRange = sum % 15 + 1;
  return valueInRange;
}
